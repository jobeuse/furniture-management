...
if(isset($_POST['Save'])){
    ...
    if(!empty($Quantity) or !empty($ImportDate) or !empty($FurnitureId)){
        ...
        if(mysqli_num_rows($select)>0){
            ...
            $total=$Quantity + $fetchs['Quantity'];

            $updateproduct=mysqli_query($conn, "UPDATE import set Quantity='$total',ImportDate='$ImportDate' where FurnitureId='$FurnitureId' ")or die('errror');
            if ($updateproduct) {
               echo "<script>alert('furniture imported   ')</script>";
            }else{
                echo "<script>alert('Qantity could not update')</script>";
            }

        }else{
            $mysql=mysqli_query($conn,"INSERT into import values('','$FurnitureId','$Quantity','$ImportDate')")or die('error');
                if($mysql){
                    echo "<script>alert('furniture imported ')</script>";
                }else{
        ...
        //getting add through url to be used to select element chosen
           ...
           //checking if the import with id selected, is in table 
            ...
            if ($id == $fetchs['id']) { 
                ...
                $delete=mysqli_query($conn, "DELETE FROM import WHERE id='$id'")or die('error');
